...
export default function EditarMembroScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    setPontoReferencia(endereco.pontoReferencia || '');

    setAniversario(membro.aniversario || '');
    setMinisterio(membro.ministerio || '');
    setCelula(membro.celula || '');
  }, [membro]);
  ...
  const handleSalvar = () => {
    ...
    axios.put(`http://192.168.18.151:3000/api/membros/${membro.id}`, membroAtualizado)
      .then(() => {
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Button title="Salvar Alterações" onPress={handleSalvar} color="#3B82F6" />
      </ScrollView>
    </KeyboardAvoidingView>
  );
}
